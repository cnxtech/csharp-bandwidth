...
namespace Bandwidth.Net.Catapult
{
  ...
  internal class AccountApi : ApiBase, IAccount
  {
    ...
    public IEnumerable<AccountTransaction> GetTransactions(AccountTransactionQuery query = null,
      CancellationToken? cancellationToken = null)
    {
      return new LazyEnumerable<AccountTransaction>(Api,
        () =>
          Api.MakeJsonRequestAsync(HttpMethod.Get, $"/users/{Api.UserId}/account/transactions", cancellationToken, query));
    }
  }
  ...
  public class AccountTransactionQuery
  {
    ...
    /// <summary>
    ///   Used for pagination to indicate the size of each page requested for querying a list of transactions. If no value is
